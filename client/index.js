...
function generateRows(books) {
  ...
  if (books.length === 0) {
   ...
   <td colspan="6" class="px-6 py-4 border-b text-center">Tidak ada buku yang ditemukan</td>
    ...
    //Jangan lupa untuk ganti BookId dengan id dari book yang sedang di looping simpan row yang dibuat ke variabel rows
    books.forEach((book) => {
      rows += `
      <tr class="book-item">
...
async function loadPage() {
  switch (currentPage) {
    case 'home':
      ...
      //panggil function generateRows dengan parameter books dan simpan hasilnya ke variabel rows kemudian isi innerHTML dari tableBody dengan rows
    ...
    case 'edit':
      ...
      //panggil function generateEditFormInput dan simpan hasilnya ke variabel formInput kemudian isi innerHTML dari form dengan formInput
...
async function addBook(book) {
  try {
    //tambahkan buku baru ke http://localhost:3333/books dengan method POST body yang dikirim adalah book yang dikirimkan sebagai parameter function
    const response = await fetch('http://localhost:3333/books',{
      method: 'POST',
      headers: { 
...
async function editBook(book) {
  try {
      //ubah buku yang ada di http://localhost:3333/books/:id dengan method PUT body yang dikirim adalah book yang dikirimkan sebagai parameter function
...
async function deleteBook(bookId) {
  try {
      //hapus buku yang ada di http://localhost:3333/books/:id dengan method DELETE id buku yang akan dihapus dikirimkan sebagai parameter function
      const response = await fetch(`http://localhost:3333/books/${bookId}`,{
        method: 'DELETE',
      });
      const removeBook = await response.json();
          return removeBook;
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menghapus buku');
